...
| Frontend       | Turbo, Stimulus, jsbundling-rails (esbuild)          |
| JS Bundling    | esbuild, pnpm (package manager)                      |
...
- **Associations**: `belongs_to :user`, `has_many :calendar_entries, dependent: :destroy`
...
- esbuild (via `jsbundling-rails`) bundles `app/javascript/application.js` into `app/assets/builds/`
- pnpm is the package manager (see `package.json`, `pnpm-lock.yaml`)
- Stimulus controllers live in `app/javascript/controllers/` and are auto-registered via `index.js`
- Build output (`app/assets/builds/*`) is gitignored; only `.keep` is tracked
- Dev server runs both Rails and the JS watch build via `bin/dev` (Procfile.dev)
...
- [x] Add `has_many :calendar_entries` to Route model
- [x] Set up JavaScript bundling (esbuild + Stimulus via pnpm)
...
# Start dev server (runs Rails + JS watch build concurrently)
bin/dev

# Or run Rails server only (build JS first)
pnpm run build
bin/rails server
```
...
- JavaScript: `app/javascript/application.js` (entry point)
- Stimulus controllers: `app/javascript/controllers/`
- JS build output: `app/assets/builds/` (gitignored, generated by esbuild)
- Migrations: `db/migrate/`
- Routes config: `config/routes.rb`
- Devise config: `config/initializers/devise.rb`
- Package config: `package.json`, `pnpm-lock.yaml`
- Dev process: `Procfile.dev`, `bin/dev`
